...
struct UWB_Dist_0 UWBDist0;
struct UWB_Dist_1 UWBDist1;
struct UWB_Dist_2 UWBDist2;
struct UWB_Dist_3 UWBDist3;
...
unsigned char UWB_DATA_BUFF[UWB_LEN];
unsigned char UWBCnt = 0;
uint8_t UWBD0[9],UWBD1[9],UWBD2[9],UWBD3[9];
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
	...
	{
		UWBData_Analysis((unsigned char)aRxCmd_2);
...
void UWBData_Analysis(unsigned char ucData)
{

	UWB_DATA_BUFF[UWBCnt++]=ucData;
	if (UWB_DATA_BUFF[0]!=0x6D)
	{
		UWBCnt=0;
		return;
	}
	if(UWBCnt<42){return;}
	else if(UWB_DATA_BUFF[1] != 0x63)
	{
		UWBCnt=0;
		return;	
	}
	...
	{
	  /*memcpy(&UWBDist0, &UWB_DATA_BUFF[6], 8);
		memcpy(&UWBDist1, &UWB_DATA_BUFF[15], 8);
		memcpy(&UWBDist2, &UWB_DATA_BUFF[24], 8);
		memcpy(&UWBDist3, &UWB_DATA_BUFF[33], 8);
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			UWBD0[uwb_index]=UWBDist0.nuwb0[uwb_index];
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			UWBD1[uwb_index]=UWBDist1.nuwb1[uwb_index];
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			UWBD2[uwb_index]=UWBDist2.nuwb2[uwb_index];
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			UWBD3[uwb_index]=UWBDist3.nuwb3[uwb_index];
		}		*/
		int8_t uwb_data_temp;
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			memcpy(&uwb_data_temp, &UWB_DATA_BUFF[6+uwb_index], 1);
			UWBD0[uwb_index]=uwb_data_temp;
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			memcpy(&uwb_data_temp, &UWB_DATA_BUFF[15+uwb_index], 1);
			UWBD1[uwb_index]=uwb_data_temp;
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			memcpy(&uwb_data_temp, &UWB_DATA_BUFF[24+uwb_index], 1);
			UWBD2[uwb_index]=uwb_data_temp;
		}
		for(int uwb_index = 0; uwb_index < 8; uwb_index++)
		{
			memcpy(&uwb_data_temp, &UWB_DATA_BUFF[33+uwb_index], 1);
			UWBD3[uwb_index]=uwb_data_temp;
		}		
	}
