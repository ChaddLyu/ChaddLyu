...
//  printf("Gyroscope data test!!!\n");
  ...
  while (1)
  {
			if(HAL_GetTick() - angle_output_tick >= 1000)  // 每1000ms输出1次数据，修改成100为100ms输出一次
			{
...
//				printf("==> Object Distance Detected : %3.3f! <==\n",jsn_dist);
				printf("UWB0:%s, UWB1:%s, UWB2:%s, UWB3:%s\n",UWBD0,UWBD1,UWBD2,UWBD3);
				//printf("UWB0:%s\n",UWBD0);
			}
    /*			
		if(bumper_event == 1)
			{
				bumper_event = 0;
				while(HAL_GPIO_ReadPin(Bumper_GPIO,Bumper_GPIO_PIN) == RESET)//读取引脚电平，低电平为碰撞到
				{
